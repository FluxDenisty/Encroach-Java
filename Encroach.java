...
public class Encroach extends JPanel{
	...
	//Listeners
	KeyListener keyActions = new KeyListener() {
	...
	MouseListener clickActions = new MouseListener() {
	...
	public Encroach() {
		...
		this.addMouseListener(clickActions);
	}
	...
	public void paintTop(){
		...
		bg.drawString(player1.get_name() + ": " + Integer.toString(player1.get_score()), 10, 60);
		bg.drawString("Games Won: ",10,80);
		bg.drawString(player2.get_name() + ": " + Integer.toString(player2.get_score()), BOARD_WIDTH/2 + 10, 60);
	...
	void setChoices(){
		...
		int opposite = player2.get_color();
		if (choiceLeft == opposite){
			choiceLeft--;
		}
		if (choiceLeft < 0 && opposite != numColours - 1){
			choiceLeft = numColours - 1;
		}else if (choiceLeft < 0){
			choiceLeft = numColours - 2;
			
		}if (choiceRight == opposite){
			choiceRight++;
		}	
		if (choiceRight > numColours -1 && opposite != 0){
			choiceRight = 0;
